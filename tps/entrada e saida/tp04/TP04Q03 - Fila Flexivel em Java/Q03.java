...
class Queue {
	...
	void insert(Game game) {
		...
		if (this.elements == this.size) this.remove();
		...
		System.out.println(Math.round((float)sum/(float)(elements)));
	}
	...
	void show() { 
		...
		for (Node tmp = first.next; tmp != last.next; tmp = tmp.next, i++) { System.out.print("[" + i + "] "); tmp.game.print(); }
	}
}
