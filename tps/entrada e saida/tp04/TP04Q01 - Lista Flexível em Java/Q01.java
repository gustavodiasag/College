...
class DynamicList {
	...
	DynamicList() { head = new Node(); tail = head; }

	public void insert(int index, Game game) {

		Node tmp = new Node(game), current = head;
	...
	public void insertLast(Game game) {

		tail.next = new Node(game);
		tail = tail.next;
	}
	...
	public void show() {
		...
		for (Node tmp = head.next; tmp != null; tmp = tmp.next, i++) { System.out.print("[" + i + "] "); tmp.game.print(); }
	}	
}
