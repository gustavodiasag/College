...
class Q05 {
	...
	public static void main(String[] args) throws Exception {
		...
		StaticList list = new StaticList(70);
		...
		for (int i = 0; i < operations; i++) {
			...
			switch (input) {
				...
				case "II": list.insertFirst(getGame(games, MyIO.readInt())); break;

				case "IF": list.insertLast(getGame(games, MyIO.readInt())); break;

				case "I*": list.insert(MyIO.readInt(), getGame(games, MyIO.readInt())); break;

				case "RF": list.removeLast(); break;
				
				case "RI": list.removeFirst(); break;

				case "R*": list.remove(MyIO.readInt()); break;

				case "RF*": list.remove(MyIO.readInt()); break;
			}
		}
...
class StaticList {
	...
	StaticList(int size) { this.games = new Game[size]; this.elements = 0; }


	public void insert(int index, Game game) {

		for (int i = elements; i > index; i--) this.games[i] = this.games[i - 1];
	...
	public void insertFirst(Game game) {
		...
		for (int i = elements; i > 0; i--) this.games[i] = this.games[i - 1];
	...
	public void show() {
		...
		for (int i = 0; i < elements; i++) { System.out.print("[" + i + "] "); games[i].print(); }
	}	
}
