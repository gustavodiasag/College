...
class Q07 {
	...
	public static void main(String[] args) throws Exception {
		...
		while (!endOfTest(input = MyIO.readLine())) queue.insert(getGame(games, Integer.parseInt(input)));
		...
		for (int i = 0; i < operations; i++) {
			...
			switch (commands[0]) {
				...
				case "I": queue.insert(getGame(games, Integer.parseInt(commands[1]))); break;

				case "R": System.out.println("(R) " + queue.remove().getName()); break;
			}
		}
	...
	public static boolean endOfTest(String input) { return (input.compareTo("FIM") == 0); }
...
class Queue {
	...
	Queue(int size) { this.games = new Game[size]; this.first = this.last = sum = 0; }

	void insert(Game game) {
		
		if (this.games[last % this.games.length] != null) this.remove();		

		this.games[(last++) % this.games.length] = game;

		sum += game.getAvgPt();

		System.out.println(Math.round((float)sum/(float)(last - first)));
	}

	Game remove() {

		Game removed = this.games[(first++) % this.games.length];
		
		this.games[(first - 1) % this.games.length] = null;

		sum -= removed.getAvgPt();
	...
	void show() {
		...
		for (int i = (first % this.games.length); this.games[i] != null; i++) {
			
			System.out.print("[" + i + "] "); this.games[i].print();
		}
	}
}
