...
class Q04 {
	...
	public static void main(String[] args) throws Exception {
		...
		// Loading the selected game names to a smaller structure
		...
		while (!endOfTest(input = MyIO.readLine())) {

			for (int i = 0; i < pos; i++) {
				
				int key = Integer.parseInt(input);

				if (games[i].getAppId() == key) { names[index++] = games[i].getName(); break; }
			}
		}
		
		// Array needs to be sorted when using the binary search algorithm
		...
		// Does the search for each name selected by the input
		while (!endOfTest(input = MyIO.readLine())) {

			totalComparisons += binarySearch(names, input);
		}

		time = (new Date()).getTime() - start;

		sendToFile(time, totalComparisons);
	}
	...
	// Chosen based on an analisys of the stdin IDs' order
	public static void quickSort(String[] names, int left, int right) {
		
		int i = left, j = right, pivot = hash(names[(left + right)/2]);

		while (i <= j) {

			while (hash(names[i]) < pivot) i++;
			while (hash(names[j]) > pivot) j--;
		...
		if (left < j) quickSort(names, left, j);
		if (right > i) quickSort(names, i, right);
	}

	// Recursive
	public static int binarySearch(String[] names, String name) {
		
		int left = 0, right = names.length - 1, middle, value = hash(name);
		...
		while (left <= right) {
			...
			if (value == hash(names[middle])) {

				System.out.println("SIM"); return comparisons;
			}
         	 
			else if (value > hash(names[middle])) left = middle + 1;
	...
	// Sum based on each character value and their position
	public static int hash(String name) {

		int constant = 13, hash = 0, power = 0;

		for (int i = 0; i < name.length(); i++) hash += (name.charAt(i) * (int)Math.pow(constant, power++));

		return hash;
	}

	public static void sendToFile(long time, int comparisons) {

		try {

			PrintWriter writer = new PrintWriter("matrícula_binaria.txt");
