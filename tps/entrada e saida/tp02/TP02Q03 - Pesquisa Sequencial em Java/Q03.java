...
class Q03 {
	...
	public static void main(String[] args) throws Exception {
		...
		// Loading the selected games to a smaller structure
		Game[] gamesToLoad = new Game[50];
		...
		while (!endOfTest(input = MyIO.readLine())) {
			...
			for (int i = 0; i < pos; i++) {
				...
				if (games[i].getAppId() == key) { gamesToLoad[index++] = games[i].clone(); break; }
			}
		}
		...
		while (!endOfTest(input = MyIO.readLine())) {
			...
			totalComparisons += sequentialSearch(input, gamesToLoad);
		}
	...
	public static int sequentialSearch(String value, Game[] games) {
		...
		for (int i = 0; i < games.length; i++) {
			...
			if (games[i].getName().compareTo(value) == 0) { System.out.println("SIM"); return comparisons; }
		}
	...
	public static void sendToFile(long time, int comparisons) {
		...
		try {
			...
			PrintWriter writer = new PrintWriter("matrícula_sequencial.txt");
