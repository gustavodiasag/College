...
void sort() {
	...
	for (uint8_t h = ELEMENTS/2; h > 0; h /= 2)	for (uint8_t i = h; i < ELEMENTS; i++) {
		
		Game tmp = GAMES[i]; MOVEMENTS++;

		uint8_t j;
		
		for (j = i; j >= h && GAMES[j - h].app_id > tmp.app_id; j -= h) { GAMES[j] = GAMES[j - h]; COMPARISONS++; MOVEMENTS++; }

		COMPARISONS++;

		GAMES[j] = tmp; MOVEMENTS++;
	}
}

void send_to_file(double duration) {
	
	FILE *out = fopen("matrícula_shellsort.txt", "w");
