...
void sort(uint8_t i) {
	...
	for (uint8_t j = i + 1; j < ELEMENTS; j++) {

		COMPARISONS++;

		if (strcmp(GAMES[lowest].name, GAMES[j].name) >= 0) lowest = j;
	}

	Game tmp = GAMES[lowest];
	GAMES[lowest] = GAMES[i];
	GAMES[i] = tmp;

	MOVEMENTS += 3;

	if (i + 1 < ELEMENTS) sort(i+1);
}

void send_to_file(double duration) {
	
	FILE *out = fopen("matrícula_selecaoRecursiva.txt", "w");
