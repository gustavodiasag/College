...
class Q09 {
	...
	public static void main(String[] args) throws Exception {
		...
		list.sort(list.games, 0, list.elements - 1);
	...
	public static void sendToFile(long time, int comparisons, int movements) {
		...
		try {
			...
			PrintWriter writer = new PrintWriter("matrícula_mergesort.txt");
...
class StaticList {
	...
	public void sort(Game games[], int left, int right) {
		
		if (left < right) {
		
            int middle = left + (right - left) / 2;

            sort(games, left, middle);
            sort(games, middle + 1, right);

            merge(games, left, middle, right);
        }
	}

	void merge(Game[] games, int left, int middle, int right) {

        int firstSize = middle - left + 1, secondSize = right - middle;

        Game[] first = new Game[firstSize], second = new Game[secondSize];

        for (int i = 0; i < firstSize; ++i) first[i] = games[left + i];
        for (int j = 0; j < secondSize; ++j) second[j] = games[middle + 1 + j];
        
        int i = 0, j = 0, k = left;

        while (i < firstSize && j < secondSize) {

            if (first[i].getUpvotes() < second[j].getUpvotes()) games[k++] = first[i++];

            else games[k++] = second[j++];
        }

        while (i < firstSize) games[k++] = first[i++];
        while (j < secondSize) games[k++] = second[j++];
    }
...
class Game {
	...
	Game(int app_id, int age, int dlcs, int avg_pt, Date release_date, String name, String owners, String website,
		 String developers, String[] languages, String[] genres, float price, float upvotes, boolean windows, boolean mac, boolean linux) {
