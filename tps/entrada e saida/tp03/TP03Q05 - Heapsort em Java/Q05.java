...
class Q05 {
	...
	public static void main(String[] args) throws Exception {
		...
		while (scanner.hasNext()) {
			...
			if (date.length() > 10) games[pos].formatter = new SimpleDateFormat("MMM dd, yyyy", Locale.US);

			else games[pos].formatter = new SimpleDateFormat("MMM yyyy", Locale.US);

			games[pos].setDate(games[pos].formatter.parse(date));	
	...
	public static void sendToFile(long time, int comparisons, int movements) {
		...
		try {
			...
			PrintWriter writer = new PrintWriter("matrícula_heapsort.txt");
...
class StaticList {
	...
	public void sort() {
		...
		for (int i = 1; i < elements; i++) toHeap(i);

		int switcher = elements;

		while (switcher > 0) {
			
			--switcher;

			Game tmp = this.games[0];
			this.games[0] = this.games[switcher];
			this.games[switcher] = tmp;
	...
	public void toHeap(int limit) {
		
		for (int i = limit; this.games[i].getDate().after(this.games[(i-1)/2].getDate()); i = (i-1)/2) {
			
			Game tmp = this.games[i];
			this.games[i] = this.games[(i-1)/2];
			this.games[(i-1)/2] = tmp;
		}
	}

	public void rearrange(int limit) {
		
		int i = 0;

		while (i < limit/2) {

			int child = getHighestChild(i, limit);
			
			comparisons++;

			if (this.games[child].getDate().after(this.games[i].getDate())) {
				
				Game tmp = this.games[i];
				this.games[i] = this.games[child];
				this.games[child] = tmp;
	...
	public int getHighestChild(int i, int limit) {
		
		int largest = i, left = 2*i+1, right = 2*i+2;
		
		if (left < limit && this.games[left].getDate().after(this.games[largest].getDate())) largest = left;
		if (right < limit && this.games[right].getDate().after(this.games[largest].getDate())) largest = right;
...
class Game {
	...
	Date getDate() { return this.release_date; }
	String getDateString() { return formatter.format(this.release_date); }
	...
	void printDate() {
		...
		String date = formatter.format(this.release_date);

		String formatted = date.substring(0, 3) + "/" + date.substring(date.length() - 4, date.length());
	...
	void print() {
		...
		System.out.printf("%d %s ", this.app_id, this.name);

		printDate();

		System.out.printf("%s %d ", this.owners, this.age);
